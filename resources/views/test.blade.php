          ...
          <table class="table table-striped" border="2">
            ...
            <tr>
              <td style="text-align:center"><b>Completions</b>
                <div align="justify">This part consists of sentences with a words missing in each. For each question, choose the answer which best completes the sentence. </div>
              </td>

              @for($i=1; $i < 4; $i++) @if(!empty($results['completions'][$i]['result'])) <td> {!! $results['completions'][$i]['right'] !!} / {!! $results['completions'][$i]['all'] !!} <br> result
                ...
                <td>
                  <form method="get" action="/english/analiz">
                ...
                @else
                <td>
                  <form method="get" action="english/completions">
            ...
            <tr>
              <td style="text-align:center"><b>Restatements</b>
                <div align="justify"><br>There are several sentences, each followed by four possible ways of restating the main idea of the sentence in different words. For each question choose the one restatement which best expresses the meaning of the original sentence.</div>
              </td>
              @for($i=1; $i < 4; $i++) @if(!empty($results['restatements'][$i]['result'])) <td> {!! $results['restatements'][$i]['right'] !!} / {!! $results['restatements'][$i]['all'] !!} <br> result
                ...
                @else
                <td>
                  <form method="get" action="english/restatements">
                    <button type="submit" class="btn btn-primary">Go</button>
                  </form>
                </td>
                @endif
            ...
            <tr>
              <td style="text-align:center"><b>Reading</b>
                <div align="justify">There is a passage followed by related questions. For each question, choose the most appropriate answer based on the text.</div>
              </td>
              @for($i=1; $i < 4; $i++) @if(!empty($results['reading'][$i]['result'])) <td> {!! $results['reading'][$i]['right'] !!} / {!! $results['reading'][$i]['all'] !!} <br> result
                ...
                @else
                <td>
                  <form method="get" action="english/reading">
                    <button type="submit" class="btn btn-primary">Go</button>
                  </form>
                </td>
                @endif
            ...
            <tr>
              ...
              @for($i=1; $i < 4; $i++) @if(!empty($results['logic'][$i]['result'])) <td> {!! $results['logic'][$i]['right'] !!} / {!! $results['logic'][$i]['all'] !!} <br> result
                ...
                @else
                <td>
                  <form method="get" action="english/logic">
                    <button type="submit" class="btn btn-primary">Go</button>
                  </form>
                </td>
                @endif
